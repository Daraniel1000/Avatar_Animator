 ...
 * Modifications by Maciej Kapuściński, 2023
 * 
...
public class OneEuroFilter
{
    ...
    public float Filter(float value, float timestamp = -1.0f)
    {
        ...
        float dvalue = x.hasLastRawValue() ? (value - x.hatXPrev()) * freq : 0.0f; // FIXME: 0.0 or value? 
        float edvalue = dx.filterWithAlpha(dvalue, alpha(dcutoff));
        // use it to update the cutoff frequency
        float cutoff = mincutoff + beta * Mathf.Abs(edvalue);
        // filter the given value
        return currValue = x.filterWithAlpha(value, alpha(cutoff));
    }
};
